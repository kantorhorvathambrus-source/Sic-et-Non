...
| 2026-09-12 | 19 | The Ecklund quotation demoted to a labelled paraphrase | "Nearly 50 percent of them are religious" is the publisher's summary of her finding, not a sentence of hers. A blurb printed inside quotation marks under a living researcher's name is the exact failure this site guards against, and disclosing it in the locator was not enough — the level was wrong. |
| 2026-09-18 | all | A person's name takes one form per locale file — prose, `quote.author` and `sources[].author` alike | Topic 6 looked like it held a convention (English `quote.author`, localised `sources[]`), but English 6 uses one form in both fields. The split was a translation oversight in all four locales, not a rule, and following it would have propagated it. Titles keep the opposite rule: original plus a gloss, because a title is a retrieval key. |
| 2026-09-18 | 6 | The Answers in Genesis Statement of Faith was credited to Whitcomb and Morris in hu, es, fr and de | The `sources[]` author values had shifted by one row during translation. Two real people were named as authors of a document they did not write. Fixed in all four, and `verify-quotes.mjs` now fails on any translated bibliography author that is neither the English name nor a registered localisation of it. |
| 2026-09-18 | all | The paraphrase-disclaimer guard is now a pattern per language | It was an English-only regex, so it rejected all ten Hungarian paraphrases even though every one of them carried the disclaimer correctly. A locale with no registered pattern now fails on its first paraphrase rather than passing unchecked. |
| 2026-09-18 | 6 | The four translations brought to parity with the English variant retrofit | The three Christian positions moved from the old `context` block into `variants`, keeping their existing translations, which still matched the current English word for word. `evolutionary-creation` was not dropped: English promoted it to the parent argument, where the Dobzhansky quote already carried it in every locale. Only the Gould variant and the new objection prose were translated. |
